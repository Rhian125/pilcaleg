...
<div class="content-wrapper">
	...
	<div class="box">
		...
		@can('read-data')
			<div class="table-responsive box-body">
				...
				<table class="table table-bordered">
					...
					<tr>
						<td>{{ ($kelurahan ->currentpage()-1) * $kelurahan ->perpage() + $loop->index + 1 }}</td>
						<td>
                            @if ($v->subdistricts_id)
								{{ $v->kecamatan->subdistricts_name}}
							@endif
						</td>
						<td>{{ $v->village_districts_name }}</td>
						<td>
							@can('ubah-data')
		...
		<div class="box-footer">
			<!-- PAGINATION -->
			<div class="float-right">{{ $kelurahan->appends(Request::only('search'))->links() }}</div>
		</div>
	</div>
	</section>
</div>
