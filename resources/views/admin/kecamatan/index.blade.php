...
<div class="content-wrapper">
	...
	<div class="box">
			...
			<div class="box-tools pull-right">
				<div class="form-inline">
					<form action="{{ url('/'.Request::segment(1).'/search') }}" method="GET">
						<div class="input-kecamatan margin">
							<input type="text" class="form-control" name="search" placeholder="Masukkan kata kunci pencarian">
							<span class="input-kecamatan-btn">
		...
		@can('read-data')
			<div class="table-responsive box-body">
				...
				<table class="table table-bordered">
					...
					<tr>
						<td>{{ ($kecamatan ->currentpage()-1) * $kecamatan ->perpage() + $loop->index + 1 }}</td>
						<td>{{ $v->subdistricts_name }}</td>
						<td>
							@can('ubah-data')
		...
		<div class="box-footer">
			<!-- PAGINATION -->
			<div class="float-right">{{ $kecamatan->appends(Request::only('search'))->links() }}</div>
		</div>
	</div>
	</section>
</div>
